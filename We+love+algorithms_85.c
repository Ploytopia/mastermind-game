...
int main() {
 ...
 // for (int game = 1; game <= 100; game++) {
    ...
    while (correctPositions[count - 1] != 5) {
      count = count + 1;
      int eligible_codes[60][CODE_LENGTH];
      int generation = 1;
      int eligible_codes_count = 0;
      ...
      while ((generation <= MAX_GENERATIONS &&eligible_codes_count <= POPULATION_SIZE) ||(eligible_codes_count == 0)) {
        for (int i = 0; i < 75; i++) {
          ...
          if (fitness(guess, child1, correctPositions, correctColors, count) !=
              0) {
            child1_is_new = false;
          }
          if (fitness(guess, child2, correctPositions, correctColors, count) !=
              0) {
            child2_is_new = false;
          }
